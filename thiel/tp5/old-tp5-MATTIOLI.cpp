...
class My {
  public:
    ...
    void set_new_masque() 
    {
        if (num_masque==M_D4) num_masque=M_D8;
        if (num_masque==M_D4) num_masque=M_2_3;
        if (num_masque==M_D4) num_masque=M_3_4;
        if (num_masque==M_D4) num_masque=M_5_7_11;
    }
    
    DemiMasque * dm = new DemiMasque(num_masque);
...
//DemiMasque dm(M_D4);
//std::vector<Ponderation> list_pond; unsigned int t_pond; NumeroMasque num_masque; std::string nom_masque;
void calculer_Rosenfeld_DT (cv::Mat img_niv)
{
    //DemiMasque * dm = new DemiMasque(M_D4);
    for (int y = 0; y < img_niv.rows; y++)
    for (int x = 0; x < img_niv.cols; x++)
    {
